...
(function() {
  var Log, PROJECT_PATH, Project, USER_PATH, User, add, alreadyHasImage, ask, auth, buildUrl, codeToApiResolution, compareFiles, download, existsFile, fileMove, filedel, fs, grab, help, https, humanResolutionToApiResolution, humanResolutionToCode, init, install, isNumeric, isValidResolutionCode, jsonfile, loadWorkspace, log, logSettings, md5File, mkdir, persist, persistProject, persistUser, printAttribution, printTitleCard, project, promptDirectoryCreation, readline, redactApiKey, remove, request, requestJson, user, validateApiKey, validateId, yn;
  ...
  validateId = function(id, callback) {
    var url;
    url = buildUrl(user.apiKey, id, (isNumeric(id) ? 'tiny' : 'large'));
    return requestJson(url, function(error, response, body) {
      return callback(!error && response.statusCode === 200);
    });
  };

  alreadyHasImage = function(id, resolution, destination) {
    var duplicates;
    duplicates = project.images.filter(function(obj) {
      return (obj.id === id && obj.resolution === resolution) || obj.destination === destination;
    });
  ...
  add = function(args) {
    ...
    if (typeof id === 'undefined' || typeof resolution === 'undefined' || typeof destination === 'undefined') {
      ...
      if (alreadyHasImage(id, resolution, destination)) {
        return log.warning('A conflicting dependency is already present in your' + ' project!');
      } else {
  ...
  validateApiKey = function(key, callback) {
    var url;
    url = buildUrl(key, '195893', 'tiny');
    return requestJson(url, function(error, response, body) {
      var success;
      success = !error && response.statusCode === 200;
      if (success) {
        url = buildUrl(key, 'bb4b32cd96264150', 'large');
        return requestJson(url, function(error, response, body) {
          return callback(true, !error && response.statusCode === 200);
        });
      } else {
    ...
    return existsFile(USER_PATH, function(err, stats) {
      ...
      if (stats) {
        ...
        return ask('What\'s your Pixabay API key? To find it you can log in to the' + ' Pixabay website and visit: https://pixabay.com/api/docs/ ', function(answer) {
          return validateApiKey(answer, function(success, elevated) {
            if (success) {
              if (!elevated) {
                log.warning('WHILE THIS API KEY WORKED, IT DOESN\'T HAVE' + ' ELEVATED PERMISSIONS. Ask Pixabay for permission to' + ' to access high quality images or you\'re limited to' + ' small images only.');
              }
